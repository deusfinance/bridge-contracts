...
contract("MuonV01", (accounts) => {
  ...
  before(async () => {
    ...
    await deaToken.mint(accounts[1], toWei('25000'))
    ...
    // for test we set this chainId
    await ethBridge.ownerSetChainId(ETH)
    await bscBridge.ownerSetChainId(BSC)
    await ftmBridge.ownerSetChainId(FTM)
  ...
  describe('Validate signatures', async () => {
      ...
      truffleAssert.eventEmitted(actionResult1, 'Action', (ev) => {
        return (ev.requestId.eq(toBN(requestId)))
      })
  	})
  ...
  describe('Bridge test', async () => {
      ...
      try {
        ...
        assert(false, `Burn occured but its not permitted`)
      } catch(error){
